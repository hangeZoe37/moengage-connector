...
For more information on URL encoding, please see this: <http://www.w3schools.com/tags/ref_urlencode.asp>.
...
#### 6.1 Single Recipient Request

```bash
https://domainname/fe/api/v1/send?username=username&password=APIpassword&unicode=true/false&from=sendercli&to=torecipient&text=message&dltContentId=DLTtemplateID
```

#### 6.2 Multiple Recipient Request (Up to 100 MSISDN)
...
|----------|-------------|--------|
| `"statusCode":2051` "Sender [xxxxxx] doesn't exist" | When the API request contains non-registered SENDER ID on User Panel |
| `"statusCode":2070` | Authentication failure | Invalid Username/Password/Account expired |
| `"statusCode":2054` Invalid Msisdn [xxxxxxxxx] [IN] | When the MSISDN is not in 10- or 12-digits length (with IN country code 91) |
...
```bash
http://{clienturl path}?txid={transactionId}&to={recpient}&from={sender}&description={description}&pdu={totalPdu}&text={message}&deliverystatus={status}&deliverydt={doneDate}
```
...
-d '{"extra":{"dltContentId": "ContentID"},
 "message": {
...
### Flash SMS Using JSON Method API
...
#### 1. One2One Request & Response Details
...
<dltPrincipalEntityId>222222222</dltPrincipalEntityId>
...
#### 2. OneToMany Request & Response Details
...
Username that register on panel. For Promotional user name use test.pro and for transactional messages use test.trans
...
Sender ID (DLT Register Header Name) from which message needs to be originated. It should be Alphabet or Numeric of 6 characters only.
