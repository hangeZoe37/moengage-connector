 ...
 * @param {object} payload - MoEngage-formatted callback payload
...
async function dispatch(dlrUrl, payload, callbackData, payloadType) {
  ...
  for (let attempt = 1; attempt <= RETRY_CONFIG.MAX_ATTEMPTS; attempt++) {
    try {
      ...
      // --- MOENGAGE CALLBACK PAYLOAD START ---
      logger.info(`--- [MOENGAGE CALLBACK PAYLOAD START] ---`);
      logger.info(JSON.stringify(payload, null, 2));
      logger.info(`--- [MOENGAGE CALLBACK PAYLOAD END] ---`);
